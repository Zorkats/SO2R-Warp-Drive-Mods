...
namespace SO2R_Warp_Drive_Mods
{
    public static class RuntimeConfigManager
    {
        ...
        {
            Plugin.Logger.LogInfo("RuntimeConfigManager initialized.");
        }
        ...
        public static void RefreshResources()
        {
            ...
            if (_cachedFont == null)
            {
                ...
                {
                    foreach(var t in allTxt) { if(t.font != null) { _cachedFont = t.font; break; } }
                }
            }
        }
        ...
        public static void Update()
        {
            ...
            if (Keyboard.current.f9Key.wasPressedThisFrame) ToggleWindow();

            if (IsVisible && _window != null) HandleInput();
        }
        ...
        private static void ToggleWindow()
        {
            ...
            if (IsVisible)
            {
                ...
                if (_window == null)
                {
                    if (_parentCanvas != null && _cachedFont != null)
                        CreateConfigWindow(_parentCanvas.transform, _cachedFont);
                    else
                    {
                        Plugin.Logger.LogWarning("Cannot open Config: No UI Canvas or Font found yet.");
                        IsVisible = false;
                        return;
                    }
                }
        ...
        private static void ToggleGameInput(bool enable)
        {
            if (enable)
            {
                Time.timeScale = 1f;
        ...
        private static void HandleInput()
        {
            ...
            bool next = kb.pageDownKey.wasPressedThisFrame || kb.rightBracketKey.wasPressedThisFrame || kb.tabKey.wasPressedThisFrame;
            bool shift = kb.leftShiftKey.isPressed || kb.rightShiftKey.isPressed;

            if (next)
            {
                if (shift) { _currentPage = (_currentPage - 1 + _totalPages) % _totalPages; ShowPage(_currentPage); }
                else       { _currentPage = (_currentPage + 1) % _totalPages; ShowPage(_currentPage); }
            }
        ...
        private static void CreateConfigWindow(Transform parent, TMP_FontAsset font)
        {
            ...
            CreateText("[F9] Close | [1-0, Q-T] Toggle | Arrows: Adjust Sliders | [TAB] Next Page", 16, new Vector2(0, 280), container.transform, font);
        ...
        private static void CreateConfigPages(TMP_FontAsset font)
        {
             var page1 = CreatePage("Page1");
                ...
                CreateToggle("No Heal on Level Up", "NoHealOnLevelUp", Plugin.EnableNoHealOnLevelUp, generalGrid, font);

                CreateSectionHeader("Difficulty Tuning", page1, font);
                var diffGrid = CreateGridContainer(page1);
                CreateSlider("Enemy Stats", "EnemyStatMult", Plugin.EnemyStatMultiplier, 0.5f, 5.0f, diffGrid, font);
                CreateSlider("Global EXP", "GlobalExp", Plugin.GlobalExpMultiplier, 0.1f, 5.0f, diffGrid, font);
                CreateSlider("Global FOL", "GlobalFol", Plugin.GlobalFolMultiplier, 0.1f, 5.0f, diffGrid, font);

                CreateSectionHeader("Quality of Life", page1, font);
                var qolGrid = CreateGridContainer(page1);
                CreateToggle("Enable BGM Info", "BgmInfo", Plugin.EnableBgmInfo, qolGrid, font);
                CreateToggle("Show BGM Once", "ShowOncePerSession", Plugin.ShowOncePerSession, qolGrid, font);
                CreateToggle("Enable Speed Mult", "MovementMultiplier", Plugin.EnableMovementMultiplier, qolGrid, font);
                CreateSlider("Move Speed", "MovementSpeed", Plugin.MovementSpeedMultiplier, 1f, 4.0f, qolGrid, font);
        ...
        private static void ToggleOption(string key)
        {
             switch (key)
            {
             ...
             if (_toggles.ContainsKey(key)) { var t = _toggles[key].GetComponent<Toggle>(); t.isOn = !t.isOn; }
            UpdateUIValues();
        }
        ...
        private static void AdjustSlider(float delta)
        {
             if (_selectedIndex >= 0 && _selectedIndex < _optionKeys.Count)
            {
                ...
                switch (key)
                {
                    case "MovementSpeed": Plugin.MovementSpeedMultiplier.Value = Mathf.Clamp(Plugin.MovementSpeedMultiplier.Value + delta, 1f, 4.0f); UpdateSliderVisual(key, Plugin.MovementSpeedMultiplier.Value); break;
                    case "EnemyStatMult": Plugin.EnemyStatMultiplier.Value = Mathf.Clamp(Plugin.EnemyStatMultiplier.Value + delta, 0.5f, 5.0f); UpdateSliderVisual(key, Plugin.EnemyStatMultiplier.Value); break;
                    case "GlobalExp": Plugin.GlobalExpMultiplier.Value = Mathf.Clamp(Plugin.GlobalExpMultiplier.Value + delta, 0.1f, 5.0f); UpdateSliderVisual(key, Plugin.GlobalExpMultiplier.Value); break;
                    case "GlobalFol": Plugin.GlobalFolMultiplier.Value = Mathf.Clamp(Plugin.GlobalFolMultiplier.Value + delta, 0.1f, 5.0f); UpdateSliderVisual(key, Plugin.GlobalFolMultiplier.Value); break;
                    case "FormationBonusPoint": Plugin.FormationBonusPointMultiplier.Value = Mathf.Clamp(Plugin.FormationBonusPointMultiplier.Value + delta, 0.1f, 1.0f); UpdateSliderVisual(key, Plugin.FormationBonusPointMultiplier.Value); break;
                    case "ChainBattleBonus": Plugin.ChainBattleBonusMultiplier.Value = Mathf.Clamp(Plugin.ChainBattleBonusMultiplier.Value + delta, 0.0f, 1.0f); UpdateSliderVisual(key, Plugin.ChainBattleBonusMultiplier.Value); break;
                    case "MissionReward": Plugin.MissionRewardMultiplier.Value = Mathf.Clamp(Plugin.MissionRewardMultiplier.Value + delta, 0.1f, 1.0f); UpdateSliderVisual(key, Plugin.MissionRewardMultiplier.Value); break;
                }
            }
        }
        ...
        private static void UpdateSliderVisual(string key, float val)
        {
             if (_sliders.ContainsKey(key))
            {
                ...
                s.value = val;
                _sliderLabels[key].text = $"{GetSliderLabel(key)}: {val:F2}";
            }
        }
        ...
        private static void UpdateUIValues()
        {
            ...
            if (_sliders.ContainsKey("EnemyStatMult")) _sliders["EnemyStatMult"].SetActive(true);
            if (_sliders.ContainsKey("GlobalExp")) _sliders["GlobalExp"].SetActive(true);
            if (_sliders.ContainsKey("GlobalFol")) _sliders["GlobalFol"].SetActive(true);
        ...
        {
             switch (key) {
                case "MovementSpeed": return "Move Speed";
                case "EnemyStatMult": return "Enemy Stats";
                case "GlobalExp": return "EXP Rate";
                case "GlobalFol": return "FOL Rate";
                case "FormationBonusPoint": return "Bonus Points";
                case "ChainBattleBonus": return "Chain Bonus";
                case "MissionReward": return "Reward Mult";
                default: return key;
            }
        }
        ...
        {
            for (int i = 0; i < _optionKeys.Count; i++) { var key = _optionKeys[i]; if (_sliderLabels.ContainsKey(key)) { _sliderLabels[key].color = (i == _selectedIndex) ? Color.yellow : Color.white; } }
        }

        private static string GetKeyHint(string key)
        {
             switch (key) {
                case "PauseOnFocusLoss": return "[1]"; case "BgmInfo": return "[2]"; case "ShowOncePerSession": return "[3]"; case "MovementMultiplier": return "[4]"; case "NoHealOnLevelUp": return "[5]"; case "FormationBonusReset": return "[7]"; case "FormationBonusHalved": return "[8]"; case "FormationBonusHarder": return "[9]"; case "FormationBonusDisable": return "[0]"; case "ChainBattleNerf": return "[Q]"; case "ChainBattleDisable": return "[W]"; case "MissionRewardNerf": return "[E]"; case "NerfAllMissions": return "[R]"; case "DebugMode": return "[T]"; default: return "";
            }
        }

        public static void Cleanup() { if (_window != null) UnityEngine.Object.Destroy(_window); }
    }
}